...
package com.zhangshuai.library;
...
public class MultiPagerSlidingTabStrip extends LinearLayout {
	...
	private int tabIndicatorMaginTop = 4;
...
//				scrollToChild(currentPosition, 0);
	...
	private void addTextIconTab(final int position, String title, int resId) {
		...
		Log.e(TAG, "addTextIconTab: linearLayout.getChildCount()"+linearLayout.getChildCount() );
	...
	private void setCurrentSelectItem(int position) {
		if (position < 3) {
            ...
            for (int i = 0; i < childAt.getChildCount(); i++) {
                ...
                if (v instanceof LinearLayout) {
                    View tabTextView = ((LinearLayout) v).getChildAt(0);
                    View tabImageView = ((LinearLayout) v).getChildAt(1);
